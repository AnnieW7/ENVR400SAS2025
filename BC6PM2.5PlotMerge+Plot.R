...
# Create scatter plot with adjusted annotation position and color by date
plot <- ggplot(data, aes(x = `temp (C)`, y = BC6, color = as.factor(Date))) +
  geom_point(alpha = 0.6, size = 2) +  # color is now mapped to Date
  annotate("text", 
           x = min(data$`temp (C)`, na.rm = TRUE) + 2.5,  # nudged right
           y = max(data$BC6, na.rm = TRUE) - 50,          # nudged down
